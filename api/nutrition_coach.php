...
// Nutrition data voor de laatste 7 dagen
// ==== Nutrition Data (laatste 7 dagen) ====
$nutritionDataContext = '';
$q3 = mysqli_query(
    $db,
...
if ($q3) {
    ...
    while ($r = mysqli_fetch_assoc($q3)) {
        ...
        if (!empty($r['created_at'])) {
            $parts[] = 'created_at=' . date('Y-m-d H:i', strtotime($r['created_at']));
        }
        if (!empty($r['fruit']))      $parts[] = 'fruit=' . $r['fruit'];
        if (!empty($r['vegetables'])) $parts[] = 'vegetables=' . $r['vegetables'];
        if (!empty($r['carbs']))      $parts[] = 'carbs=' . $r['carbs'];
        if (!empty($r['dairy']))      $parts[] = 'dairy=' . $r['dairy'];
        if (!empty($r['protein']))    $parts[] = 'protein=' . $r['protein'];
    ...
    if ($lines) {
        $nutritionDataContext =
            "Voedingsinvoer van de gebruiker (laatste 7 dagen, nieuwste eerst):\n"
            . implode("\n", $lines)
            . "\n\nGebruik deze informatie om advies te baseren op wat de gebruiker daadwerkelijk heeft ingevoerd over fruit, groenten, koolhydraten, zuivel en eiwitten.";
    }
}

// Voeg toe aan $messages net als meals-context
if ($nutritionDataContext !== '') {
    array_unshift($messages, ['role' => 'system', 'content' => $nutritionDataContext]);
}
...
echo json_encode([
    ...
    '__nutrition_ctx' => $nutritionDataContext,
]);
